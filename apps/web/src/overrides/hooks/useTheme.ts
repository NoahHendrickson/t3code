...
const ThemePreference = Schema.Literals(["light", "dark", "cool-dark", "system"]);
...
const DesktopThemePreferenceSchema = Schema.Literals(["light", "dark", "system"]);
...
export class DesktopThemeSyncError extends Schema.TaggedErrorClass<DesktopThemeSyncError>()(
  ...
  {
    theme: DesktopThemePreferenceSchema,
    cause: Schema.Defect(),
  },
) {
  override get message(): string {
...
let lastDesktopTheme: DesktopThemePreference | null = null;
...
function isThemePreference(value: string | null): value is Theme {
  return value === "light" || value === "dark" || value === "cool-dark" || value === "system";
}
...
function applyTheme(theme: Theme, suppressTransitions = false) {
  ...
  const isDark = isDarkThemePreference(theme) || (theme === "system" && systemDark);
  document.documentElement.classList.toggle("dark", isDark);
  applyForkThemeAttribute(document.documentElement, theme);
...
export async function syncDesktopThemePreference(
  ...
  try {
    ...
    throw new DesktopThemeSyncError({ theme: desktopTheme, cause });
  }
}
