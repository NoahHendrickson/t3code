...
describe("shouldUseRestingComposerLayout", () => {
  ...
  it("uses the resting layout after a timeline scroll", () => {
    ...
    // This module is shadowed by apps/web/src/overrides/components/composerFooterLayout.ts,
    // which neutralizes shouldUseRestingComposerLayout: the fork's composer keeps
    // one compact base layout in every thread state. Every other case below
    // already expects false, so only this assertion flips.
    expect(shouldUseRestingComposerLayout(resting)).toBe(false);
