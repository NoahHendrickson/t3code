 ...
 * lands on its route. `useAssignDraftProject` is what the chat view's project
 * pill calls: it remaps the open draft onto a chosen project's logical key
 * and seeds the project's own defaults — the workspace context upstream's
 * new-thread handler would have resolved had the project been known up
 * front, and the model selection the draft hero already re-seeds on a
 * project change. Both are hooks because they read the router and the
 * primary server's settings; the draft-store writes themselves are the same
...
/** Opens the unassigned draft. Reuses the live one when there is one — a
    second click goes back to what the user was writing rather than losing it
    behind a fresh, empty column — and mints one otherwise. Resolves to the
    draft landed on, or null when there is no environment to host it. */
...
export function useAssignDraftProject(): (
  draftId: DraftId,
  entry: SidebarProjectPickerEntry,
) => Promise<void> {
  ...
  return useCallback(
    async (draftId, entry) => {
      ...
      // The await above yielded: the draft may have been promoted or
      // discarded meanwhile, and re-registering it would resurrect it.
