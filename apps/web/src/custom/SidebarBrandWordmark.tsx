/**
 * Pixel "no3y" wordmark from Figma t3-fork node 364:11245 (Union 387:21624).
 * A 13×3 cell grid drawn at 8px tall, currentColor at 40% so Light and Dark
 * both keep the lockup's muted name without a baked-in white.
 */
export function SidebarBrandWordmark() {
  return (
    <svg
      aria-hidden
      className="h-2 w-[34.667px] shrink-0 text-sidebar-foreground/40"
      fill="currentColor"
      shapeRendering="crispEdges"
      viewBox="0 0 13 3"
    >
      <rect height="1" width="1" x="0" y="0" />
      <rect height="1" width="1" x="1" y="0" />
      <rect height="1" width="1" x="4" y="0" />
      <rect height="1" width="1" x="7" y="0" />
      <rect height="1" width="1" x="8" y="0" />
      <rect height="1" width="1" x="10" y="0" />
      <rect height="1" width="1" x="12" y="0" />
      <rect height="1" width="1" x="0" y="1" />
      <rect height="1" width="1" x="2" y="1" />
      <rect height="1" width="1" x="5" y="1" />
      <rect height="1" width="1" x="8" y="1" />
      <rect height="1" width="1" x="11" y="1" />
      <rect height="1" width="1" x="0" y="2" />
      <rect height="1" width="1" x="2" y="2" />
      <rect height="1" width="1" x="4" y="2" />
      <rect height="1" width="1" x="8" y="2" />
      <rect height="1" width="1" x="10" y="2" />
    </svg>
  );
}
