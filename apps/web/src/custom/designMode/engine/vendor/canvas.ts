...
export interface CanvasModeOpts {
  /** Fit math only — the RIGHT panel's inset, since that's the one the artboard fits beside. */
  dock: { mode(): 'docked' | 'floating'; width(): number }
  /** Canvas took (true) / released (false) the page. Its own opt, not a `dock` method: the
   * receivers are every panel that pushes the page (both docks today), which has nothing to
   * do with the fit math above — conflating them forced the host to synthesize a fake Dock
   * just to fan one bit out to two objects (PR #45 review). */
...
export class CanvasMode {
  ...
  private unapply(): void {
    ...
    this.saved = null
    this.opts.onCanvasActive(false)
