...
export const UTILITY_PREFIXES: Record<string, string> = {
  ...
  'min-height': 'min-h',
  'max-height': 'max-h',
  // P3 insets — Tailwind v4 sizes left/top off the same numeric spacing scale as padding/margin
  // (`left-6`, `top-10`, and negatives as `-left-2`), so they need no branch of their own, just
  // a prefix. Both directions of the absolute story read them: the `absolute` op's inset ask
  // (request.ts) and the plain `left`/`top` css drafts an already-absolute element gets
  // (delegated call #2). `right`/`bottom` stay out until something drafts them.
  left: 'left',
  top: 'top',
