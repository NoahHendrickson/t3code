...
import { type RowSpec, GAP_SPEC, SIZE_MODES, defeatFillIfGrowing, isEffectivelyAbsolute } from './panel-specs'
...
export interface LayoutSectionDeps {
  ...
  /** Multi-select flag, live (refresh() already receives it per call; this is for the CLICK
   * handlers). The Absolute toggle is single-selection-only — P3 ships single-selection
   * structural ops, multi lands in P5 — so its handler must refuse regardless of how the click
   * arrived, not merely because refresh() hid the button. */
  isMulti: () => boolean
...
export class LayoutSection {
  ...
  // Position block widget (P3) — the block itself (label + X/Y rows) is panel.ts's, but this
  // toggle is a stateful control with the same per-selection lifecycle as the cluster's own
  // widgets, so it lives here rather than adding a third refresh/teardown track to panel.ts.
  private absoluteToggle: HTMLButtonElement | null = null
  ...
  /**
   * The Absolute toggle (P3, ratified #4) — lives in the Position block's header, right of the
   * `Position` group label, and is what lights up the X/Y fields. Pressed state rides BOTH the
   * `.on` class (the overlay's existing pressed-class vocabulary — `.canvas-toggle.on`, and what
   * `.position-absolute-toggle.on` in the stylesheet paints) and `aria-pressed` (every other
   * panel toggle: wrap, align). No new convention is minted for either half.
   *
   * One click = one op that owns the parent too (ratified #4): applyAbsolute records the parent
   * and whether it still needs `position: relative`, so there is nothing extra to do here.
   */
  buildAbsoluteToggle(): HTMLButtonElement {
    const btn = createButton({ label: '⌖' })
    btn.classList.add('position-absolute-toggle')
    btn.setAttribute('data-absolute-toggle', '')
    btn.setAttribute('aria-label', 'Absolute position')
    btn.setAttribute('aria-pressed', 'false')
    btn.title =
      "Absolute position — take this element out of flow (position: absolute + left-*/top-*), making its parent a positioning context (relative) if it isn't already"
    btn.addEventListener('click', () => this.onAbsoluteToggle())
    this.absoluteToggle = btn
    return btn
  }

  /** Absolute toggle click. Pressed state IS the current effective state, so the next state is
   * its negation — and applyAbsolute's own collapse rule turns a toggle straight back into a
   * dropped draft rather than a stacked no-op ask, which is why this doesn't special-case
   * "undo". Refuses in multi-select (see deps.isMulti). */
  private onAbsoluteToggle(): void {
    if (this.deps.isMulti()) return
    this.withEdit((el) => {
      this.deps.drafts.applyAbsolute(el, !isEffectivelyAbsolute(el, this.deps.drafts))
    })
  }
   ...
   * inferred current mode) only exist for flex children that are IN FLOW — Fill/Hug are flex
   * concepts and updateSizeMode only runs there. Min/Max/Variable are universal.
   *
   * The out-of-flow half of that gate is PR #46 review finding 8, and reuses refreshFlexChild's
   * own predicate (isEffectivelyAbsolute) rather than a second copy: an absolutely-positioned
   * child isn't laid out by its flex parent, so picking Fill drafted a dead `flex-grow: 1` that
   * still rode the request as a contradictory ask alongside the absolute op. The whole trio goes,
   * not just Fill — updateSizeMode stops running for an out-of-flow child (refreshFlexChild
   * returns before it), so `sm.mode` is stale there and the checkmark would be lying about which
   * mode is current. */
  sizeMenuItems(spec: RowSpec, hasVariable: boolean): MenuItem[] {
    ...
    const inFlow = el !== null && !isEffectivelyAbsolute(el, this.deps.drafts)
    if (el && inFlow && parent && isFlex(parent as TaggedElement) && sm) {
  ...
  refresh(el: TaggedElement, computed: CSSStyleDeclaration, multi: boolean): void {
    if (multi) {
      ...
      // The Absolute toggle is single-only too (P3 ships single-selection structural ops; multi
      // lands in P5). panel.ts hides the whole Position block in multi — offsets across N
      // elements are noise — but the button carries its own hidden flag so its visibility is a
      // local invariant, not something that only holds because an ancestor happens to hide.
      if (this.absoluteToggle) this.absoluteToggle.hidden = true
    ...
    this.refreshAbsoluteToggle(el)
    this.refreshLayoutSection(el, computed)
    this.refreshFlexChild(el, computed)
    this.refreshMinMax(el, computed)
  }

  /** Pressed state of the Absolute toggle — the effectively-absolute predicate, shared with
   * X/Y's editability and the flex-child hide gate (panel-specs.ts's isEffectivelyAbsolute), so
   * a pressed toggle and editable X/Y fields can never disagree. */
  private refreshAbsoluteToggle(el: TaggedElement): void {
    if (!this.absoluteToggle) return
    const on = isEffectivelyAbsolute(el, this.deps.drafts)
    this.absoluteToggle.hidden = false
    this.absoluteToggle.classList.toggle('on', on)
    this.absoluteToggle.setAttribute('aria-pressed', String(on))
  }
  ...
  private refreshFlexChild(el: TaggedElement, computed: CSSStyleDeclaration): void {
    const parent = el.parentElement
    // An absolutely-positioned child is OUT OF FLOW: its flex parent doesn't lay it out, so
    // align-self (and the Fill/Hug modes below, which are flex concepts) mean nothing — Figma
    // hides the same controls for an absolute element. DOM stays for stable order, same rule as
    // the multi branch above.
    const visible = parent !== null && isFlex(parent as TaggedElement) && !isEffectivelyAbsolute(el, this.deps.drafts)
