...
/* t3-fork: chat-styles.ts is not vendored (the chat/session feed is replaced by the T3
 * composer, and the panel is native T3 UI since the headless split) — CHAT_CSS is simply
 * gone from the concat below. */
import { CANVAS_VERBS_CSS } from './canvas-verbs-styles'
...
// page content is pushed right by LeftDock's html margin-left (dock.ts).
`.layers-panel {
