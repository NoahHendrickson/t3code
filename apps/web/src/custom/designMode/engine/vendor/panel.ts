...
export class Panel {
  ...
  show(elOrEls: TaggedElement | TaggedElement[], data: InspectorData): void {
    ...
    this.emptyEl.hidden = true
    /* t3-fork: promptButton attached the selection as a chat chip to the Forge's own
     * composer; T3's thread composer replaces that surface, so the button stays hidden. */
    this.promptButton.hidden = true
    if (els.length > 1) {
  ...
  refresh(): void {
    ...
    for (const { field, spec } of this.fields) {
      ...
      if (spec.read) {
        ...
        // Editability is LIVE state (P3): the Absolute toggle flips X/Y between editable and
        // read-only through a refresh(), so the disabled flag is asserted here on every pass
        // rather than once in buildField. Never editable in multi — the Position block hides
        // there anyway, and P3's structural ops are single-selection only (multi is P5).
        const input = field.root.querySelector('input') as HTMLInputElement
        input.disabled = !this.rowEditable(spec, el, multi)
        continue
      }
  ...
  /**
   * Whether a `readOnly` row (Position X/Y) currently accepts edits — the ONE gate behind both
   * the disabled flag in refresh() and the commit guard below, so a disabled input and an
   * inert-but-live scrub gesture can never disagree. A row with no `write` is permanently
   * read-only; multi-select never edits (see refresh()'s note).
   */
  private rowEditable(spec: RowSpec, el: TaggedElement, multi: boolean): boolean {
    return !multi && !!spec.write && (spec.editable?.(el, this.drafts) ?? false)
  }

  /**
   * Commit path for a `readOnly` row's own value (Position X/Y): not a css property, so the
   * generic `commit` would draft a fake `x`/`y`. Re-checks editability at CALL time — the field's
   * onInput closure is built once per selection, while the Absolute toggle changes the answer
   * mid-selection, and a label scrub reaches this even while the input is disabled.
   */
  private commitReadOnlyRow(spec: RowSpec, n: number): void {
    const el = this.el
    if (!el || !this.rowEditable(spec, el, this.isMulti())) return
    this.onBeforeEdit(el)
    spec.write!(el, n, this.drafts)
    this.refresh()
    this.onEdited()
  }
  ...
  private buildBody(): void {
    ...
    for (const section of SECTIONS) {
      ...
      if (section.custom === 'layout') {
        ...
        // Offsets from offsetParent; P3 made the fields conditionally EDITABLE (POSITION_ROWS'
        // `editable`/`write`, driven by the Absolute toggle in the header below). Declared in
        // panel-specs.ts and built through buildRow like the Size block's rows, so refresh +
        // destroy ride the standard field lifecycle (PR #44 follow-up) — the live editability
        // flip is a refresh(), never a rebuild; only the multi-hide toggle needs the block element.
        const posBlock = document.createElement('div')
        posBlock.className = 'position-block'
        posBlock.setAttribute('data-position-block', '')
        // Header row (mirrors .align-head): the group label plus the Absolute toggle to its
        // right. LayoutSection owns the button's state/teardown like every other cluster widget.
        const posHead = document.createElement('div')
        posHead.className = 'position-head'
        const posLabel = document.createElement('span')
        posLabel.className = 'group-label'
        posLabel.textContent = 'Position'
        posHead.append(posLabel, this.layoutSection.buildAbsoluteToggle())
        ...
        posBlock.append(posHead, posFields)
  ...
  private buildField(spec: RowSpec): BoundField {
    ...
    const field = new NumberField({
      ...
      // `readOnly` rows (Position X/Y) don't hold a css property of their own, so they never take
      // the generic commit — which would draft a fake 'x'/'y'. commitReadOnlyRow routes to the
      // spec's `write` (setAbsoluteInset vs a plain left/top draft, see positionStateOf) and is
      // inert while the row is in its read-only state, for typed values AND label scrubs alike.
      onInput: spec.readOnly ? (n) => this.commitReadOnlyRow(spec, n) : commit,
    ...
    // Disabled is the safe default at BIRTH only — refresh() (which show() runs immediately
    // after buildBody) owns the live value, since P3's Absolute toggle can flip it without a
    // rebuild. Set here so a field is never briefly typeable before the first refresh.
