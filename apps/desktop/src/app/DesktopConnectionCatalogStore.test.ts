...
describe("DesktopConnectionCatalogStore", () => {
  ...
  it.effect("surfaces catalog filesystem failures instead of treating them as missing", () =>
    Effect.gen(function* () {
      ...
      const permissionError = PlatformError.systemError({
        ...
        pathOrDescriptor: `${baseDir}/userdata-fork/connection-catalog.json`,
      });
      ...
      assert.equal(error.catalogPath, `${baseDir}/userdata-fork/connection-catalog.json`);
      assert.strictEqual(error.cause, permissionError);
      assert.equal(
        error.message,
        `Failed to read the desktop connection catalog at ${baseDir}/userdata-fork/connection-catalog.json.`,
      );
  ...
  it.effect("reports the failed catalog write operation and path", () =>
    Effect.gen(function* () {
      ...
      const permissionError = PlatformError.systemError({
        ...
        pathOrDescriptor: `${baseDir}/userdata-fork`,
      });
      const fileSystemLayer = Layer.succeed(
        FileSystem.FileSystem,
        FileSystem.makeNoop({
      ...
      assert.equal(error.path, `${baseDir}/userdata-fork`);
      assert.strictEqual(error.cause, permissionError);
      assert.equal(
        error.message,
        `Desktop connection catalog write failed during create-directory at ${baseDir}/userdata-fork.`,
      );
      assert.notEqual(error.message, permissionError.message);
    }).pipe(Effect.provide(NodeServices.layer), Effect.scoped),
  );
  ...
  it.effect("surfaces a catalog that can no longer be decrypted without deleting it", () =>
    Effect.gen(function* () {
      ...
      assert.equal(
        error.message,
        `Desktop connection catalog protection failed during decrypt-catalog at ${baseDir}/userdata-fork/connection-catalog.json.`,
      );
